import { BullionRates, KaratType, MakingChargeConfig, PriceBreakdown, Product } from "./types";
...
export const PricingEngine = {
  ...
  GST_RATE: 0.03, // 3% GST (HSN 7113)

  /**
   * Get dynamic per-gram gold rate for a specific karat based on 24K benchmark
   */
  getKaratRate(karat: KaratType, base24kRate: number): number {
  ...
  calculateBreakdown(
    product: Product,
    selectedKarat?: KaratType,
    bullionRates?: BullionRates,
    customDiscountPct: number | null = null,
    ...
    let makingType: "percent" | "per_gram" = "percent";
   ...
   */
  calculateOldGoldValue(weightGrams: number | string, karat: KaratType, bullionRates?: BullionRates) {
