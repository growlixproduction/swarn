  ...
  return (
    <div className="pdp-live-breakdown-card" id="pdp-live-breakdown-box">
      ...
      <div className="breakdown-card-body" id="pdp-breakdown-card-body">
        ...
        <div className="pdp-making-customizer">
          <div className="making-cust-header">
            <span className="making-cust-title">
              <i className="fa-solid fa-sliders"></i> Adjust Making Charges:
            </span>
            <div className="making-mode-tabs">
              <button
                type="button"
                className={`making-mode-btn ${isPercent ? "active" : ""}`}
              ...
              <button
                type="button"
                className={`making-mode-btn ${!isPercent ? "active" : ""}`}
                onClick={() => handleTypeToggle("per_gram")}
              >
                ₹ / Gram
              </button>
            </div>
          ...
          <div className="making-input-row">
            <div className="making-stepper-box" style={{ minWidth: "160px", padding: "0.25rem 0.5rem" }}>
              <button type="button" className="making-step-btn" onClick={() => handleStep(-1)} title="Decrease" style={{ width: "32px", height: "32px", fontSize: "1.1rem" }}>
                -
              </button>
              <input
                type="number"
                ...
                step={isPercent ? 1 : 50}
                style={{ fontWeight: 800, fontSize: "1.1rem", width: "55px", textAlign: "center", border: "none", padding: "0 2px" }}
                onChange={handleInputChange}
              />
              <span className="making-unit-label" style={{ fontWeight: 700, paddingRight: "0.25rem" }}>{isPercent ? "%" : "₹/g"}</span>
              <button type="button" className="making-step-btn" onClick={() => handleStep(1)} title="Increase" style={{ width: "32px", height: "32px", fontSize: "1.1rem" }}>
                +
              </button>
            </div>
            ...
            <div className="making-presets-strip">
              {presets.map(pVal => (
                <button
                  key={pVal}
                  type="button"
                  className={`making-preset-pill ${makingVal === pVal ? "active" : ""}`}
                  onClick={() => setProductMakingCharge(product.id, { value: pVal })}
                >
                  {isPercent ? `${pVal}%` : `₹${pVal}`}
                </button>
              ))}
          ...
          <div className="making-helper-note">
            <i className="fa-solid fa-circle-info"></i> Adjust weight or making charge to live recalculate total invoice.
          </div>
        </div>
